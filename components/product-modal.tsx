...
export function ProductModal({
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center px-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-[1] max-h-[88%] w-full max-w-[340px] overflow-hidden rounded-2xl bg-white shadow-xl">
        ...
        <div className="max-h-[calc(88vh-64px)] overflow-y-auto px-5 pb-5 pt-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <button
            type="button"
            onClick={onAdd}
            className="mt-5 w-full rounded-xl bg-brand-emerald py-3.5 text-[15px] font-semibold text-white transition-colors hover:brightness-95 active:scale-[0.99]"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="mt-2.5 w-full rounded-xl bg-[#eef0f5] py-3.5 text-[15px] font-semibold text-brand-coral transition-colors hover:bg-[#e6e9f0]"
          >
            Fechar
          </button>
        </div>
